...
serve(async (req) => {
  ...
  try {
    const { apartmentId, checkIn, checkOut, discountCode, guestEmail } = await req.json()
    ...
    if (nights <= 0) return new Response(JSON.stringify({ error: 'Date non valide' }), { status: 400, headers: { ...cors, 'Content-Type': 'application/json' } })
    ...
    if (smoobuApiKey && smoobuId) {
      try {
        const r = await fetch(`https://login.smoobu.com/api/rates?apartments[]=${smoobuId}&start_date=${checkIn}&end_date=${checkOut}`, {
        ...
        if (r.ok) {
          ...
          if (rates) {
            const prices = Object.entries(rates as Record<string, { price: number }>)
              ...
              .map(([, v]) => v.price)
            if (prices.length > 0) pricePerNight = Math.round(prices.reduce((a, b) => a + b, 0) / prices.length)
          }
        }
    ...
    if (!stripeKey) return new Response(JSON.stringify({ error: 'Stripe non configurato' }), { status: 500, headers: { ...cors, 'Content-Type': 'application/json' } })

    console.log('Creating PaymentIntent, key prefix:', stripeKey.substring(0, 10))
    ...
    console.log('Stripe response status:', stripeRes.status, '| id:', pi.id)

    if (!stripeRes.ok) {
      console.error('Stripe error:', pi?.error?.message)
      return new Response(JSON.stringify({ error: pi?.error?.message ?? 'Errore Stripe' }), { status: 502, headers: { ...cors, 'Content-Type': 'application/json' } })
    }
